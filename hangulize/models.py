...
class Notation(object):
    ...
    def __iter__(self):
        if not getattr(self, '_rewrites', None):
            self._rewrites = [Rewrite(*item) for item in list(self.items())]
...
class Language(object):
    ...
    def transcribe(self, string, logger=None):
        ...
        # flatten
        phonemes = reduce(list.__add__, list(map(list, [_f for _f in phonemes if _f])), [])
        return phonemes
...
class Rewrite(object):
    ...
    def __call__(self, string, phonemes=None, lang=None, logger=None):
        ...
        if phonemes:
            deletions = []
        def repl(match):
            val = self.val(match, self) if isinstance(self.val, collections.Callable) else self.val
            repls.append(val)
                ...
                if not is_tuple:
                    if lang:
                        ...
                        if len(srcvars) == len(dstvars) == 1:
                            ...
                            dictionary = dict(list(zip(src, dst)))
